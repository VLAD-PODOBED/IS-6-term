...
def encrypt(keyword_column : str, keyword_row : str, message: str) -> str:
    # удаляем повторяющиеся буквы в ключевых словах
    keyword_column = ''.join(sorted(set(keyword_column), key=keyword_column.index))
    keyword_row = ''.join(sorted(set(keyword_row), key=keyword_row.index))
    ...
    print('\nТаблица отстортированная по строкам')
    for row in matrix:
        print(row)
...
def decrypt(keyword_column : str, keyword_row : str, message: str):
    ...
    print('\nТаблица с восстановленными строками')
    for row in matrix:
        print(row)
    ...
    matrix = np.transpose(matrix)
    print('\nТаблица с восстановленными столбцами')
    for row in matrix:
        print(row)
